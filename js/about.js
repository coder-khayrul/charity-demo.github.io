...
document.addEventListener('DOMContentLoaded', () => {
    ...
    //TEam section script
    const single_team_card = document.querySelectorAll(".mamber_card");
    const team_card_inner = document.querySelectorAll(".mamber_card_inner")

    function updateCardWidth() {
        let team_card_width;
        single_team_card.forEach(card => {
          team_card_width = card.offsetWidth;
          console.log("Updated card width:", team_card_width);
          
          // You can add any other dynamic actions here
        });
        team_card_inner.forEach(card_inner => {
            card_inner.style.width = `calc(${team_card_width}px - 50px )`
        })
      }
    ...
    var OurTeamSlider = new Swiper(".team_slider", {

        slidesPerView: 1,
        spaceBetween: 10,
        navigation: {
            nextEl: ".swiper-button-next_tm",
            prevEl: ".swiper-button-prev_tm",
        },
        breakpoints: {
            576: {
                slidesPerView: 2,
